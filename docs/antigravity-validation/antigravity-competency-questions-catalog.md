...
**Target group_id**: `cq1-synthetic-lethality`

---

## cq2: FOP Drug Repurposing

**Question**: *What other drugs targeting the BMP Signaling Pathway could be repurposed for FOP? specifically verifying if 'Janex-1' is a valid candidate.*
...
| Janex-1 (Adversarial Trap) | CHEMBL:? | Verify: Is this truly an ACVR1 inhibitor? |
...
4. **Adversarial Check**: Determine if "Janex-1" (often confused with LDN-193189) targets ACVR1 or JAK3.
5. Identify Inhibitors: `LDN-193189` (inhibits ACVR1/BMPR1A)
...
4. **Adversarial Validation**: `chembl_search_compounds("Janex-1")` -> Check target profile (JAK3 vs ACVR1).
5. **Drug Search**: `chembl_search_compounds("ACVR1 inhibitor")` → Candidates
6. **Persist**: `mcp__graphiti-docker__add_memory(group_id="cq2-fop-repurposing")`
...
**Question**: *Starting with the (now obsolete) disease ID `MONDO:0014109`, identify the current NGLY1 deficiency definition and find repurposable drugs.*

**Key Entities**:
| Entity | CURIE | Role |
|--------|-------|------|
| NGLY1 | HGNC:17646 | Causal gene |
| NGLY1 deficiency | MONDO:0800044 | Target disease (Correct ID) |
| NGLY1 deficiency (Old) | MONDO:0014109 | **Adversarial Trap** (Obsolete) |
| N-glycanase pathway | WP:WP5078 | Member pathway |

**Workflow**:
1. **Adversarial Start**: Attempt to resolve `MONDO:0014109`.
2. **Self-Healing**: Detect obsolescence -> Search Web/OpenTargets for *current* ID (`MONDO:0800044`).
3. **Gene Discovery**: `hgnc_search_genes("NGLY1")` → HGNC:17646
4. **Pathway Context**: `wikipathways_get_pathways_for_gene("NGLY1")` → Member pathways
5. **Pathway Expansion**: `wikipathways_get_pathway_components()` → All pathway members
6. **Drug Search**: For each pathway protein → `chembl_search_compounds()` with target filter
7. **Persist**: `mcp__graphiti-docker__add_memory(group_id="cq7-ngly1-drug-repurposing")`
...
3. **Mechanism Extraction**: curl ChEMBL /mechanism. **Pattern A Rule**: IF empty AND Phase 3, THEN Web Search ("[Drug Name] mechanism").
...
**Validation**: `docs/antigravity-validation/competency-validations/cq13-commercial-trials-validation.md`
