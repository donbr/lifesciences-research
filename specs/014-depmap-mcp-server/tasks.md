...
- [ ] **T001** Fix omit-if-null across `src/lifesciences_mcp/models/depmap.py`. Remove
...
- [ ] **T002** Rename `DepMapModelCandidate` to `CellModel`, the name used throughout spec.md and
...
- [ ] **T003** [P] Add `_rate_limited_call` to `DepMapClient`: one request per second behind an
...
- [ ] **T004** [P] Stop using `ErrorEnvelope.upstream_error()`, which hardcodes another API's name,
...
- [ ] **T005** [P] Export `DepMapClient` from `clients/__init__.py` and the DepMap models from
...
- [ ] **T006** [P] Fix import ordering in `servers/depmap.py`, which places `from fastmcp import
...
- [ ] **T007** [P] [US1] Extend `tests/unit/test_depmap_client.py`: an exact zero delta returns
  direction `none` (FR-004). This case currently reports WT-selective.
- [ ] **T008** [P] [US1] Unit test: a cohort below `min_lines` returns `tested` false with a note and
  **no** statistics keys present in the dump (FR-003 plus omit-if-null).
- [ ] **T009** [P] [US1] Unit test: cells with missing values are dropped and the reported cohort
  sizes reflect the drop (FR-002); an all-missing cohort reports size zero.

### Implementation

- [ ] **T010** [US1] Fix the direction tie in `compute_genotype_contrast`:
  `"mutant-selective" if delta < 0 else "WT-selective"` must yield `none` when the delta is exactly
  zero.
- [ ] **T011** [US1] Add `mutation_type` and `min_lines` to `GenotypeContrast` so a result records
  the cohort definition it came from (FR-005, FR-009).
- [ ] **T012** [US1] (FR-001) Demote the vector-argument `genotype_contrast` tool to the internal
...
- [ ] **T013** [US1] Add the cached-release loader behind `genotype_contrast_by_gene` and
...
- [ ] **T014** [US1] Make `genotype_contrast_by_gene` an `async def` tool, matching every other tool
  in `servers/`.
- [ ] **T015** [US1] Add the `get_dependency` tool per `contracts/get_dependency.md`. The
...
- [ ] **T016** [P] [US2] Unit test alias normalisation over a fixture index: `A549`, `a549`,
  `MCF-7`, `MCF7`, `HeLa`, `hela` all resolve; ranking puts an exact alias match first.
- [ ] **T017** [P] [US2] Unit test: an unmatched query returns an empty `PaginationEnvelope`, not
  `ENTITY_NOT_FOUND` (FR-007).
- [ ] **T018** [P] [US2] Unit test: `get_model("A549")` returns `UNRESOLVED_ENTITY` whose recovery
  hint names `search_models`, and makes no request (FR-008).
- [ ] **T019** [P] [US2] Contract test: both list tools accept `slim` and the slim payload carries
  only `id`, `name` and `data_source` (FR-012, Constitution Principle IV).

### Implementation

- [ ] **T020** [US2] Build the local alias index in `DepMapClient`: one request to
...
- [ ] **T021** [US2] Rewrite `search_models` to rank against that index, and return an empty
  envelope rather than an error on no match.
- [ ] **T022** [US2] Add the `UNRESOLVED_ENTITY` guard to `get_model` and every other strict tool,
  before any network call.
- [ ] **T023** [P] [US2] Add `slim` to `search_models` and `models_with_mutation`, and change the
  default page size from 10 and 500 to 50.
- [ ] **T024** [P] [US2] Add `cross_references` to the cell-model schema, populated with the CCLE
...
- [ ] **T025** [P] [US3] Unit test: an unaccepted mutation type returns `INVALID_INPUT` listing the
  six accepted values, before any request.
- [ ] **T026** [P] [US3] (FR-010, FR-011) Unit test: `page_size` in the envelope is the requested size, and a cursor
  is present whenever more results remain.

### Implementation

- [ ] **T027** [US3] Make `mutation_type` required with no default on `models_with_mutation`, and
...
- [ ] **T028** [US3] Fix pagination: return the requested `page_size`, propagate the upstream `next`
...
- [ ] **T029** [US3] Echo `mutation_type` on the cohort and on any contrast derived from it.

---

## Phase 5: Integration and polish

- [ ] **T030** Add `tests/integration/test_depmap_api.py`, marked `integration` and `depmap`, with a
...
- [ ] **T031** Wire the server into `src/lifesciences_mcp/servers/gateway.py` with prefix `depmap`
...
- [ ] **T032** [P] Surface the access terms in the tool descriptions, not only in docstrings: Cell
...
- [ ] **T033** [P] Remove the unreachable branch in `mannwhitney_u_p` (the `if sigma_sq else`
  fallback after an early return already covers non-positive variance).
- [ ] **T034** [P] Update `CLAUDE.md`: add DepMap to the status list, the structure tree and the
  implemented-tools table, and note that gene-effect requires a cached release.
- [ ] **T036** [P] (FR-013, FR-014) Unit tests for the two requirements that currently have an
...
- [ ] **T037** [P] (FR-015) Unit test that consecutive client calls are spaced by the configured
  interval and that a 429 triggers backoff rather than an immediate retry, using a fake clock.
- [ ] **T038** (FR-017) Add a guard that the analysis pipeline gains no dependency on this server:
...
- [ ] **T039** (SC-002) Make the alias resolution test sample-based rather than a fixed list of six
...
- [ ] **T035** Run `uv run pytest -m depmap -v`, `uv run pytest -m unit -v`,
